...
// CHECK-LABEL: func @memref_shape_cast_dynamic
func @memref_shape_cast_dynamic(%M : memref<?x?xf32>) -> memref<?x?xvector<16xf32>> {
// CHECK:        %[[SRC:.*]] = llvm.insertvalue %{{.*}}, %{{.*}}[4, 1] : !llvm.struct<(ptr<float>, ptr<float>,
...
// CHECK-NEXT:   %[[size2:.*]] = llvm.udiv %[[src_size2]], %[[VEC_WIDTH]]
// CHECK-NEXT:   %[[cum_size:.*]] = llvm.mul %[[size1]], %[[size2]]
// CHECK-NEXT:   %[[st2:.*]] = llvm.mlir.constant(1 : index)
// CHECK-NEXT:   %[[st1:.*]] = llvm.mul %[[st2]], %[[size2]]
// CHECK-NEXT:   llvm.insertvalue %[[size1]], %{{.*}}[3, 0] : !llvm.struct<(ptr<vec<16 x float>>, ptr<vec<16 x float>>, i64, array<2 x i64>, array<2 x i64>)>
// CHECK-NEXT:   llvm.insertvalue %[[st1]], %{{.*}}[4, 0] : !llvm.struct<(ptr<vec<16 x float>>, ptr<vec<16 x float>>, i64, array<2 x i64>, array<2 x i64>)>
// CHECK-NEXT:   llvm.insertvalue %[[size2]], %{{.*}}[3, 1] : !llvm.struct<(ptr<vec<16 x float>>, ptr<vec<16 x float>>, i64, array<2 x i64>, array<2 x i64>)>
// CHECK-NEXT:   llvm.insertvalue %[[st2]], %{{.*}}[4, 1] : !llvm.struct<(ptr<vec<16 x float>>, ptr<vec<16 x float>>, i64, array<2 x i64>, array<2 x i64>)>
// CHECK-NEXT:   llvm.return %{{.*}} : !llvm.struct<(ptr<vec<16 x float>>, ptr<vec<16 x float>>, i64, array<2 x i64>, array<2 x i64>)>
  %MV = memref_shape_cast %M : memref<?x?xf32> to memref<?x?xvector<16xf32>>
